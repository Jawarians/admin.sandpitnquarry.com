                        ...
                        <nav aria-label="Assignments pagination">
                            <ul class="pagination d-flex flex-wrap align-items-center gap-2 justify-content-center">
                                ...
                                @foreach ($assignments->getUrlRange(1, $assignments->lastPage()) as $page => $url)
                                    <li class="page-item {{ $page == $assignments->currentPage() ? 'active' : '' }}">
                                        <a class="page-link {{ $page == $assignments->currentPage() ? 'bg-primary-600 text-white' : 'bg-neutral-200 text-secondary-light' }} fw-semibold radius-8 border-0 d-flex align-items-center justify-content-center h-32-px w-32-px text-md" href="{{ $url }}">{{ $page }}</a>
                                    </li>
                                @endforeach
