...
<div class="card h-100 p-0 radius-12">
    <div class="card-header border-bottom bg-base py-16 px-24 d-flex align-items-center justify-content-between flex-wrap gap-3">
        ...
        <div class="d-flex gap-2">
            <a href="{{ route('drivers.show', $driver) }}" class="btn btn-info btn-sm radius-8">
    ...
    <div class="card-body p-24">
        ...
        <form action="{{ route('drivers.update', $driver) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row g-3">
                        ...
                        <div class="card-body p-24">
                            <div class="row g-3">
                                ...
                                <div class="col-md-12 mb-2">
                                    <div class="d-flex justify-content-center mb-3">
                                        @if($driver->user->profile_photo_path)
                                            <img src="{{ asset('storage/' . $driver->user->profile_photo_path) }}" alt="{{ $driver->user->name }}" class="w-80-px h-80-px rounded-circle object-cover">
                                        @else
                                            <img src="{{ asset('assets/images/user.png') }}" alt="{{ $driver->user->name }}" class="w-80-px h-80-px rounded-circle object-cover">
                                        @endif
                                    </div>
                                </div>
                                @endif
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', optional($driver->user)->name) }}" required>
                                    @error('name')
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="email" class="form-control @error('email') is-invalid @enderror" id="email" name="email" value="{{ old('email', optional($driver->user)->email) }}" required>
                                    @error('email')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" class="form-control @error('phone') is-invalid @enderror" id="phone" name="phone" value="{{ old('phone', optional($driver->user)->phone) }}" required>
                                    @error('phone')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                                        <option value="Active" {{ old('status', optional($driver->latest)->status) == 'Active' ? 'selected' : '' }}>Active</option>
                                        <option value="Inactive" {{ old('status', optional($driver->latest)->status) == 'Inactive' ? 'selected' : '' }}>Inactive</option>
                                        <option value="Suspended" {{ old('status', optional($driver->latest)->status) == 'Suspended' ? 'selected' : '' }}>Suspended</option>
                                    </select>
                                    @error('status')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="col-md-12">
                                    ...
                                    <select class="form-select @error('transporter_id') is-invalid @enderror" id="transporter_id" name="transporter_id" required>
                                        ...
                                        @foreach($transporters as $transporter)
                                            <option value="{{ $transporter->id }}" {{ old('transporter_id', $driver->transporter_id) == $transporter->id ? 'selected' : '' }}>
                                                {{ $transporter->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-12">
                                    ...
                                    <select class="form-select @error('truck_id') is-invalid @enderror" id="truck_id" name="truck_id">
                                        ...
                                        @foreach($trucks as $truck)
                                            <option value="{{ $truck->id }}" {{ old('truck_id', optional(optional($driver->current)->truck)->id) == $truck->id ? 'selected' : '' }}>
                ...
                <!-- Status Update -->
                <div class="col-12">
                    <h6 class="text-secondary-light fw-semibold mb-3">Status Update</h6>
                    <div class="card bg-neutral-100 radius-12 mb-4">
                        <div class="card-body p-24">
                            <div class="row g-3">
                                <div class="col-md-12">
                                    ...
                                    @error('notes')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                    <small class="form-text text-muted">Add notes about this status change (optional)</small>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        transporterSelect.addEventListener('change', function() {
            ...
            fetch(`/api/transporters/${selectedTransporterId}/trucks`)
                ...
                .then(trucks => {
                    ...
                    // If editing and there was a previously selected truck, try to reselect it
                    const currentTruckId = "{{ optional(optional($driver->current)->truck)->id }}";
                    if (currentTruckId) {
                        for (let i = 0; i < truckSelect.options.length; i++) {
                            if (truckSelect.options[i].value === currentTruckId) {
        ...
        // Trigger change event if a transporter is already selected
