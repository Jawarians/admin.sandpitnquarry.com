                        ...
                        <table class="table bordered-table sm-table mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $trip->created_at ? $trip->created_at->format('d M Y') : 'N/A' }}</td>
                                    <td>
                                        @if($trip->driver)
                                            <div class="d-flex align-items-center">
                                                <div class="flex-grow-1">
                                                    <span class="text-md mb-0 fw-normal text-secondary-light">{{ $trip->driver->name }}</span>
                                                    <br>
                                                    <small class="text-xs text-secondary-light">{{ $trip->driver->phone ?? 'No phone' }}</small>
                                                </div>
                                            </div>
                                        @else
                                    ...
                                    <td>
                                        @if($trip->truck)
                                            <div class="d-flex align-items-center">
                                                <div class="flex-grow-1">
                                                    <span class="text-md mb-0 fw-normal text-secondary-light">{{ $trip->truck->plate_number ?? $trip->truck->name ?? 'N/A' }}</span>
                                                    <br>
                                                    <small class="text-xs text-secondary-light">{{ $trip->truck->model ?? 'Unknown model' }}</small>
                                                </div>
                                            </div>
                                        @else
                                            <span class="bg-warning-focus text-warning-600 border border-warning-main px-12 py-4 radius-4 fw-medium text-sm">No Truck</span>
                                        @endif
                                    </td>
                                    <td>
                                        <div class="d-flex flex-column">
                                            @if($trip->origin && $trip->destination)
                                                <span class="text-sm text-secondary-light">{{ Str::limit($trip->origin, 20) }}</span>
                                                <iconify-icon icon="mdi:arrow-down" class="text-secondary-light text-xs"></iconify-icon>
                                                <span class="text-sm text-secondary-light">{{ Str::limit($trip->destination, 20) }}</span>
                                            @else
                                                <span class="text-sm text-secondary-light">Route not set</span>
                                            @endif
                                        </div>
                                    </td>
                                    <td>
                                        @if($trip->distance_km)
                                            <span class="text-md mb-0 fw-normal text-secondary-light">{{ number_format($trip->distance_km, 1) }} km</span>
                                        @else
