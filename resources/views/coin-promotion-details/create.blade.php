...
@php
    $title='Add Promotion Detail';
    $subTitle = 'Add a new promotion detail';
@endphp

@section('content')
    <div class="card h-100 p-0 radius-12">
        <div class="card-header border-bottom bg-base py-16 px-24">
            <h5 class="mb-0 text-lg fw-medium">Add Promotion Detail for Promotion #{{ $coinPromotion->id }}</h5>
        </div>
        <div class="card-body p-24">
            ...
            <form action="{{ route('coin-promotion-details.store') }}" method="POST">
                @csrf
                
                <input type="hidden" name="coin_promotion_id" value="{{ $coinPromotion->id }}">
                ...
                <div class="row mb-20">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" class="form-control @error('coins') is-invalid @enderror" id="coins" name="coins" value="{{ old('coins') }}" required>
                        @error('coins')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row mb-20">
                    <div class="col-md-6">
                        ...
                        <input type="number" class="form-control @error('free_coins') is-invalid @enderror" id="free_coins" name="free_coins" value="{{ old('free_coins') }}" required>
                        @error('free_coins')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="d-flex justify-content-end gap-3 mt-30">
                    <a href="{{ route('coin-promotions.edit', $coinPromotion->id) }}" class="btn btn-secondary">Cancel</a>
